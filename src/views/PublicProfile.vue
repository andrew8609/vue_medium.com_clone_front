...
<template>
    <v-container>
        ...
        <div v-for="(story, index) of publicProfile.Stories" :key=index>
            <router-link :to="viewStoryLink(story.id)" class="preview-link">
                <v-row >
                    <v-col cols="9">
                        <h6>{{story.created_at}}</h6>
                        <h4><b>{{story.title}}</b></h4>
                        <div class="brief-content" v-if="story.content_pure">{{briefStory(story.content_pure)}}</div>
                    </v-col>
                    <v-col cols="3" align="center"
                        justify="center">
                        <div v-if="getImage(story.content)" v-html="getImage(story.content)">
                        </div>
...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        store.dispatch(FETCH_PUBLICPROFILE, to.params.userId); 
        next();
    },
    computed: {
        ...mapGetters(["publicProfile"]),
    },
    methods: {
