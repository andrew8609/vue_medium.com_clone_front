                ...
                <v-row>
                    <v-col cols="2">
                        ...
                        <v-list align="center" color="transparent">
                            ...
                            <h5 style="margin-top: 50px"><b>Author Follows</b></h5>
                            <v-list dense>
                                 
                                <v-list-item v-for="follower in followeds" :key="follower.id">
                                    <router-link
                                        :to="{ name: 'user', params: { userId: follower.id } }"
                                    >
                                        <span>
                                            <v-avatar>
                                                <img style="width:40px;height:40px" :src="follower.photo_url" />
                                            </v-avatar>
                                            <span>{{briefName(follower.username)}}</span>
                                        </span>
                                    </router-link>
...
<script>
    ...
    import { FETCH_STORY, FETCH_COMMENTS, FOLLOW_USER, UNFOLLOW_USER, STORY_RESET_STATE, CLAP_STORY, FETCH_STORY_HASHTAGS, FETCH_AUTHOR_FOLLOWERS } from "@/store/actions.type";
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            ...
            Promise.all([
                ...
                store.dispatch(FETCH_STORY_HASHTAGS, to.params.storyId),
        ...
        mounted() {
            store.dispatch(FETCH_AUTHOR_FOLLOWERS, this.story.User.id);
        },
        computed: {
            ...mapGetters(["story", "comments", "isFollowing", "currentUser", "followeds"])
        },
        methods: {
            toggle() {
                this.open = !this.open;
